<div>
    <label for="fichiers" class="block text-sm font-medium text-slate-700">
        Pièces jointes <span class="font-normal text-slate-400">(facultatif — 5 fichiers max., 10 Mo par fichier)</span>
    </label>
    <input id="fichiers" type="file" wire:model="fichiers" multiple
           ...
           aria-describedby="fichiers-error fichiers-star-error"
    ...
    @error('fichiers')
        <p id="fichiers-error" class="mt-1 text-sm text-red-600">{{ $message }}</p>
    @enderror
    @error('fichiers.*')
        <p id="fichiers-star-error" class="mt-1 text-sm text-red-600">{{ $message }}</p>
    @enderror
</div>
